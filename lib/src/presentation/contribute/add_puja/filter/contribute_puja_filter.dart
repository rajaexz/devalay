...
class ContributePujaFilterWidget extends StatefulWidget {
  const ContributePujaFilterWidget({
    super.key,
  }); 

  @override
  State<ContributePujaFilterWidget> createState() =>
      _ContributePujaFilterWidgetState();
}

class _ContributePujaFilterWidgetState
    extends State<ContributePujaFilterWidget> {
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: List.generate(
                        contributePujaCubit.filterTypes.length,
                        (index) => GestureDetector(
                          onTap: () => setState(() =>
                              contributePujaCubit.selectedChipIndex = index),
                          child: Padding(
                            padding: EdgeInsets.symmetric(vertical: 10.sp),
                            child: Text(
                              contributePujaCubit.filterTypes[index],
                              style: Theme.of(context)
                                  .textTheme
                                  .bodyMedium
                                  ?.copyWith(
                                    color: contributePujaCubit
                                                .selectedChipIndex ==
                                            index
                                        ? accentColor
              ...
              ClearApplyFilter(
                ...
                applyTap: () async {
                  try {
                    ...
                    await context.read<ContributePujaCubit>().applyFilter(
                          sortBy: contributePujaCubit.selectedSortByIndex,
                          orderBy: contributePujaCubit.selectedOrderByIndex,
                        );
                ...
                clearTap: () async {
                  try {
                    ...
                    setState(() {
                      contributePujaCubit.selectedSortByIndex = null;
                      contributePujaCubit.selectedOrderByIndex =
                          ''; // Default value
                    });

                    contributePujaCubit.clearFilters();
                    contributePujaCubit.applyFilter(
                      newSectionIndex: contributePujaCubit.sectionIndex,
                    );
  ...
  Widget _buildSelectedFilterContent() {
    switch (contributePujaCubit.selectedChipIndex) {
      case 0: // Sort By
        return SizedBox(
          height: 300.h,
          child: Padding(
            ...
            child: ListView.separated(
              shrinkWrap: true,
              itemCount: contributePujaCubit.sortBy.length,
              itemBuilder: (context, index) {
                final item = contributePujaCubit.sortBy[index];
                final isSelected =
                    contributePujaCubit.selectedSortByIndex == item;

                return InkWell(
                  onTap: () {
                    setState(() {
                      contributePujaCubit.selectedSortByIndex =
      ...
      case 1: // Order By
        return SizedBox(
          ...
          child: Padding(
            ...
            child: ListView.separated(
              shrinkWrap: true,
              itemCount: contributePujaCubit.orderBy.length,
              itemBuilder: (context, index) {
                final item = contributePujaCubit.orderBy[index];
                final isSelected =
                    contributePujaCubit.selectedOrderByIndex == item['title'];
