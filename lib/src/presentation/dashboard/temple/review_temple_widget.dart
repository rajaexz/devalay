...
class _ReviewTempleWidgetState extends State<ReviewTempleWidget> {
  ...
  void initState() {
    ...
    contributeTempleCubit.sectionIndex = 4;
    ...
    if (scrollController.position.pixels >= scrollController.position.maxScrollExtent - 200 && 
      ...
      if (!contributeTempleCubit.isClosed) {
        contributeTempleCubit.fetchContributeTempleData(
          value: 'true', // Keep the same filter for review temples
          loadMoreData: true,
        ).then((_) {
    ...
    return BlocConsumer<ContributeTempleCubit, ContributeTempleState>(
      listener: (context, state) {
        if (state is ContributeTempleLoaded && mounted) {
          debugPrint('Review temple list length: ${state.templeList?.length}');
              ...
              if (mounted) {
                showDialog(
                  ...
                  builder: (context) => AlertDialog(
                    ...
                    content: Text(
                      '${StringConstant.youdonot} review temples.',
