...
class DevManage extends StatefulWidget {
  const DevManage({super.key});

  @override
  State<DevManage> createState() => _DevManageState();
}
...
class _DevManageState extends State<DevManage> {
  ...
  void initState() {
    ...
    context
        ...
        .fetchContributeDevData(approvedVal: 'true', value: "flase");
    ...
    return BlocConsumer<ContributeDevCubit, ContributeDevState>(
      listener: (context, state) {
        if (state is ContributeDevLoaded) {
          debugPrint('Draft Manage list length: ${state.model?.length}');
      ...
      builder: (context, state) {
        if (state is ContributeDevLoaded) {
          if (state.model?.isEmpty ?? true) {
            return const Center(
              child: Text('No Magnage approved  found'),
            );
          }
        }
        if (state is ContributeDevLoaded) {
