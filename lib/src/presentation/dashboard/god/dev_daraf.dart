...
class DevDaraf extends StatefulWidget {
  const DevDaraf({super.key});

  @override
  State<DevDaraf> createState() => _DevDarafState();
}

class _DevDarafState extends State<DevDaraf> {
              ...
              itemBuilder: (context, index) {
                ...
                ItemCart(
                  onTap: () {
                    ...
                    isDraft: devItem?.draft == true,
                    title: devItem?.title ?? '',
                    imageUrl: devItem?.images?.banner?.isNotEmpty == true
                    ...
                    progress: 0.5,
                        ...
                        lastEdited:
                        '${devItem?.stepsRemaining ?? 0} steps remaining ',
                    // lastEdited:
                    //     'initiated ${HelperClass().formatDate(templeItem?.createdAt ?? '')}',
                    contributeCubit: contributeDevCubit,
                    type: ContributionType.dev,
                    id: devItem?.id.toString() ?? '',
                );
              },
