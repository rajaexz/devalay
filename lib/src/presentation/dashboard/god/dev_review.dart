...
class DevReview extends StatefulWidget {
  const DevReview({super.key});

  @override
  State<DevReview> createState() => _DevReviewState();
}

class _DevReviewState extends State<DevReview> {
  ...
  void initState() {
    ...
    context.read<ContributeDevCubit>().fetchContributeDevData(value: "true");
  ...
  void scrollListener() {
    ...
    if (scrollController.position.pixels >=
      ...
      contributeDevCubit
        ...
        draftVal: '', // Keep the same filter for draft temples
        loadMoreData: true,
      )
          .then((_) {
        if (mounted) {
  ...
  void _refreshData() {
    ...
    contributeDevCubit.fetchContributeDevData(value: "ture");
  }
              ...
              itemBuilder: (context, index) {
                ...
                return ItemCart(
                  onTap: () {
                    ...
                    progress: 0.5,
                                        lastEdited:
                        'Since ${HelperClass().formatDate(templeItem?.updatedAt ?? '')}',
                    // lastEdited:
