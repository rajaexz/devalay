...
class EventApproved extends StatefulWidget {
  const EventApproved({super.key});

  @override
  State<EventApproved> createState() => _EventApprovedState();
}

class _EventApprovedState extends State<EventApproved> {
       ...
       context.read<ContributeEventCubit>().fetchContributeEventData(
          ...
          return RefreshIndicator(
            ...
            itemBuilder: (context, index) {
              ...
              return ItemCart(
                onTap: () {
                  AppRouter.push("/singleEvent/${event?.id.toString()}");
                  // AppRouter.push(
                  ...
                  progress: 0.5,
                  lastEdited: 'initiated ${HelperClass().formatDate(event?.createdAt ?? '')}',
